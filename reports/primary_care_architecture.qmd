...
subtitle: "Public-data anchored benchmark for primary care funding policy discussion"
...
**Source-confidence label: policy judgement + public-data benchmark.**

This report explains a public-data anchored benchmark for primary care funding in Aotearoa New Zealand.
...
Aggregate validation status: `public_aggregate_validated` for registered public aggregate gates only. The current release model card labels the aggregate lane `empirically_supported_if_gated`, while the not-valid-for warning still excludes precise fiscal savings, ED reductions, hospital-demand reductions, workforce effects, implementation impacts and causal effects.
...
# How the posts map to this report and dashboard {#post-map}
...
|---|---|---|---|---|
| 01 | [Post 01](#post-01) | Executive summary; comparator framing; scenario interpretation | Start here; Current state; Reference scenarios | Model-generated index, not an empirical outcome |
| 02 | [Post 02](#post-02) | Funding model explainer; formula appendix | Funding models module; Educational explainer | Public-data benchmark, not a forecast |
| 03 | [Post 03](#post-03) | Marginal supply and the extra-contact viability threshold | Microeconomics lab | Static teaching diagram, not a measured curve |
| 04 | [Post 04](#post-04) | Game theory, controls and gaming risk | Game theory lab | Strategic teaching diagram, not an estimate |
| 05 | [Post 05](#post-05) | Current reform comparator and F0/current reform pathway | Current state; Reference scenarios | F0/current reform remains the comparator |
| 06 | [Post 06](#post-06) | Controlled scheduled payment and uncapping logic | Educational explainer; Microeconomics lab | Model-generated index wording stays in place |

## Post 01. Are we buying hospital growth by rationing cheaper care upstream? {#post-01}

**Source-confidence label: executive-summary framing.**

**Accessibility description:** This post is the entry point. It links the policy question, the current reform comparator and the scenario ranking so the reader can move from the question to the benchmark without needing the dashboard first.
...
## Post 02. Fee-for-service, capitation and blended funding {#post-02}
...
## Post 03. Marginal supply {#post-03}
...
## Post 04. Why formulas do not solve games {#post-04}
...
## Post 05. Current reform pathway {#post-05}
...
This post maps to the current reform comparator section and the F0 scenario. The report keeps current reform in view so the reader can compare the public benchmark against the real policy path.

## Post 06. What I mean by uncapping primary care funding {#post-06}
...
This report is a public explanation of a model-generated benchmark. It can compare policy logics, identify assumptions and guide stakeholder validation.
...
The formal boundary is in the release model card and claim-boundary documents on this site.
...
# 6. Scenario results from the benchmark

**Source-confidence label: public-data benchmark.**

The table below uses public-data anchored model-generated indices. These are not observed New Zealand outcomes.
...
xychart-beta
  ...
  x-axis "Reference scenario" ["F4","F3","F2","F9","F8","F7","F0","F1","F6","F5"]
...
# 7. What the benchmark suggests

**Source-confidence label: public-data benchmark + policy judgement.**

The benchmark points to four patterns:
...
The next step is not a more complicated benchmark. It is testing the load-bearing assumptions:
