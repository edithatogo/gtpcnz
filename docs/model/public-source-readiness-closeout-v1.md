...
`data/public_processed/` is empty. No ETL scripts exist between raw and processed.
...
| G7 | Transformed file passes schema validation | `python -m pytest -q models/tests/test_transformed_schemas.py` |
...
| C2 | All 3 calibration targets have `source_ready=true` | `python scripts/run_public_aggregate_calibration.py --check-only` |
...
> 6. All 3 calibration targets have `source_ready=true` when `python scripts/run_public_aggregate_calibration.py --check-only` runs.
