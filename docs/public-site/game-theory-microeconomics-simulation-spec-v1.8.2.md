...
- `dynamic_streamlit`: the Streamlit module or educational simulation used for guided exploration.
...
- Use `educational teaching simulation` for interactive modules that are explanatory rather than empirical.
- Use `model-generated index` only for scenario outputs that come from the model layer.
- Do not label a educational module as a forecast, prediction, estimate, effect, saving, or demand reduction.
...
- Static visuals use the required conceptual or educational labels instead of outcome language.
- Dynamic modules are labelled as educational teaching simulations unless they are later promoted to model-generated indices.
