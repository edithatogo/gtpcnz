...
$
\begin{array}{ll}
\textbf{Flow} & \textbf{Mechanism} \\
1 & \text{Unmet need moves from primary care into urgent care, ambulance and hospitals.} \\
2 & \text{Providers choose whether to expand, maintain or ration activity.} \\
3 & \text{Patients choose whether to wait, pay, delay, use online care or go to hospital.} \\
4 & \text{Government sees hospital pressure more clearly than upstream failure.} \\
5 & \text{Intermediaries either support population health or create friction.}
\end{array}
$
...
$
\begin{array}{lll}
\textbf{Player} & \textbf{What they are trying to avoid} & \textbf{What they may do under pressure} \\
\text{Patients} & \text{Delay, cost, uncertainty, worsening illness} & \text{Wait, pay, delay, use telehealth, call ambulance, go to hospital} \\
\text{Providers} & \text{Unfunded work, burnout, financial risk} & \text{Close books, shorten appointments, raise fees, limit extra activity} \\
\text{Health New Zealand} & \text{Visible failure, deficits, hospital pressure} & \text{Prioritise urgent hospital pressures} \\
\text{Primary Health Organisations or locality bodies} & \text{Loss of role, loss of funding, accountability risk} & \text{Defend functions, manage pass-through, shape provider incentives} \\
\text{Accident Compensation Corporation} & \text{Uncontrolled claims cost, poor outcomes} & \text{Tighten payment rules or shift toward commissioning} \\
\text{Ministers} & \text{Publicly visible service failure} & \text{Fund the pressure people can see}
\end{array}
$
...
\text{Capitation} & \text{Continuity and population responsibility} \\
\text{Uncapped scheduled fee-for-service} & \text{Eligible primary medical activity} \\
\text{Place-based accountability} & \text{Prevents cherry-picking of easy activity} \\
\text{Scope-enabled supply} & \text{Lets safe care be generated by the right provider, not only the traditional provider} \\
\text{Data, audit and top-tier KPIs} & \text{Lets the system see access failure before it becomes hospital pressure}
\end{array}
$
...
1 & \text{Do scheduled activity payments increase safe primary care supply?} \\
2 & \text{Does unmet primary care need flow into urgent care, ambulance and hospitals?} \\
3 & \text{Do Accident Compensation Corporation activity payments help sustain local primary care capacity?} \\
4 & \text{Do Primary Health Organisation payment arrangements create material pass-through, transparency or entry barriers?} \\
5 & \text{Can scope-enabled providers expand supply safely and equitably?}
\end{array}
$
