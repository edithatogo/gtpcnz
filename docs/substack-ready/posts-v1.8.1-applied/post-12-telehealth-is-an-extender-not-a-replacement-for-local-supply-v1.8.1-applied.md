...
Telehealth remains modelled as an extender rather than a replacement for local supply. The public aggregate validation lane does not validate a delivery-mode effect.

**Claim boundary:** Do not claim telehealth reduces demand, replaces local workforce, or resolves rural access constraints. Those are implementation claims outside the v1.8.1 evidence status.
