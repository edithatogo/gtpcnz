...
The model is no longer only demonstrative in the public aggregate lane. It now carries public_aggregate_validated status for registered gates, while preserving explicit not-valid-for warnings.

**Claim boundary:** The model is still not linked-data calibrated and not a patient-level forecast. It remains not valid for precise fiscal savings, ED reductions, hospital-demand reductions, workforce effects, implementation impacts, or causal effects.
