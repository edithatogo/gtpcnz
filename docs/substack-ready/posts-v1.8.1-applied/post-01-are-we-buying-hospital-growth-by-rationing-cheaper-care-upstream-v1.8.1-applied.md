...
The model now has a bounded public aggregate validation lane. Registered public validation gates support the framing question more strongly than the earlier demonstrative model, but only at aggregate policy-architecture level.

**Claim boundary:** Do not frame this as proof that upstream primary care investment will reduce hospitals, ED use, or fiscal pressure. The v1.8.1 result supports a public aggregate policy-learning hypothesis, not a precise impact estimate.
