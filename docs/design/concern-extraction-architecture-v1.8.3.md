...
| **Mypy --strict** | **Prepared, not enforced** — The contract layer is fully typed, `EngineProtocol` is defined, but `--strict` over all engine modules is gated behind Track 043 completion. Ruff linting runs unconditionally. | Full static typing across engines requires refactoring that is in progress. |
