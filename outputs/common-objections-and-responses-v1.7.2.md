# Common objections and suggested responses v1.7.2
...
## Additional red-team objections added in v1.7.2

| Objection | Response |
|---|---|
| The current reform pathway may be enough. | That is possible. The series should ask whether current reforms change the marginal supply game enough, not assume failure. |
| Rural access may not be fixed by fee-for-service. | Correct. Rural access may require workforce, housing, capital, infrastructure, digital support, salaried models and rural loading. Scheduled fee-for-service is one supply lever, not the whole rural policy. |
| Scope expansion may fragment care. | That is why shared records, clinical governance, continuity metrics and place accountability are required. |
| Hospital pressure has many causes. | Yes. Ageing, workforce, technology, expectations, hospital productivity and disease burden all matter. The upstream-access hypothesis is one mechanism, not a total explanation. |
| ACC injury payments are not general illness care. | Correct. Accident Compensation Corporation is used as an analogy for rules-based treatment payment, not as a complete transplantable model. |
| If you need all these safeguards, maybe the model is too risky. | The safeguards are the model. The weak-control version is explicitly rejected. |
