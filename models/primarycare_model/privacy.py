...
def laplace_noise(scale, size=1, rng=None):
    ...
    if scale <= 0.0: return np.zeros(size)
...
def assert_aggregate_only(data):
    ...
    if d: raise ValueError(f"Patient data: {d}")
...
class PrivacyBudget:
    def __init__(self,total=1.0): self.total=total; self.spent=0.0
    @property
    def remaining(self): return max(0.0,self.total-self.spent)
    def consume(self,eps):
        if eps>self.remaining: raise ValueError(f"{eps}>{self.remaining}")
        self.spent+=eps; return eps
    def reset(self): self.spent=0.0
