...
def test_calibration_target_readiness_has_one_row_per_target() -> None:
    ...
    assert all(row.calibration_claim_status == "public_aggregate_target_ready" for row in rows)
...
def test_calibration_target_readiness_default_mode_is_non_promotional() -> None:
    ...
    assert all(row.calibration_gate_ready for row in rows)
    assert all(row.blockers == () for row in rows)
    assert all("precise fiscal savings" in row.not_valid_for for row in rows)


def test_calibration_target_readiness_strict_mode_passes_after_source_readiness() -> None:
    assert strict_calibration_target_issues() == ()
...
def test_calibration_target_readiness_cli_passes_in_readiness_mode() -> None:
    ...
    assert "public_aggregate_target_ready" in result.stdout


def test_calibration_target_readiness_cli_strict_mode_passes_after_targets_ready() -> None:
    result = subprocess.run(
