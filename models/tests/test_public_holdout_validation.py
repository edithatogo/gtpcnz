...
def test_public_holdout_comparisons_run_without_promoting_claims() -> None:
    ...
    assert {row.status for row in comparisons} == {"passed"}
    assert {row.claim_status for row in comparisons} == {"calibration_readiness_only"}


def test_public_holdout_gate_statuses_report_cal_g_003_and_cal_g_004_passed() -> None:
    assert holdout_gate_status("CAL-G-003") == "passed"
    assert holdout_gate_status("CAL-G-004") == "passed"
...
def test_public_holdout_comparison_json_has_claim_boundary() -> None:
    ...
    assert failed_rows == []
    assert all(row["tolerance_gap"] == 0 for row in payload["rows"])
    assert all(row["failing_groups"] == [] for row in payload["rows"])
    assert all(row["failing_observations"] == [] for row in payload["rows"])
    assert all(row["next_data_model_requirement"] for row in payload["rows"])
...
def test_public_holdout_cli_readiness_mode_passes() -> None:
    ...
    assert "CAL-G-004: ethnicity/Pacific; status=passed" in result.stdout
    assert "comparison_failed" not in result.stdout
    assert "tolerance_gap=" in result.stdout
    assert "failing_groups=" in result.stdout
    assert "next=" in result.stdout


def test_public_holdout_cli_require_pass_succeeds() -> None:
    result = subprocess.run(
