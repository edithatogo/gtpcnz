...
def test_dashboard_uses_explainer_and_not_forecast_language():
    ...
    assert "model-generated indices use a 0-100 scale and are unitless scores" in text
    assert "budget and payment charts use illustrative NZD" in text
    assert "appointment and activity charts use counts of appointments or eligible activity units" in text
    ...
    assert "Illustrative capitation budget constraint (NZD)" in text
    assert "Illustrative scheduled activity payment with controls (NZD)" in text
