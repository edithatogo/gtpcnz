...
You are the track lead for `058-australia-scope-decision`. Use DeepSeek v4 Flash as a narrow implementation worker. Your job is to coordinate the work packages, not broaden scope.
...
Required gates:
- `python -m pytest -q models/tests/test_jurisdiction_claims.py`

## Work-Package Subagent Prompt

You are assigned one work package from `058-australia-scope-decision`. Edit only files listed under allowed file ownership. If you need any other file, stop and write a handoff request. Preserve the public benchmark claim boundary. Run the package gate before returning.
